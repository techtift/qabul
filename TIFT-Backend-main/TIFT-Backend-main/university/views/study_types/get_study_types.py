...
from university.services.study_types.get_study_types import get_study_types


class ProgramFacultyStudyTypeResponseSerializer(serializers.Serializer):
...
class GetProgramsFacultyStudyTypesAPIView(APIView):
    @swagger_auto_schema(
        operation_summary="Get All Programs",
        responses={
            status.HTTP_200_OK: openapi.Response(
                description="List of programs",
                schema=ProgramFacultyStudyTypeResponseSerializer(many=True)
            ),
        },
        manual_parameters=[
            openapi.Parameter(
    ...
    def get(self, request):
        try:
            ...
            study_types = get_study_types(program_id=program_id,faculty_id=faculty_id)
            serializer = ProgramFacultyStudyTypeResponseSerializer(study_types, many=True)
